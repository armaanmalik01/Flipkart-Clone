...
export default function ProductSection({title, bg_color, products }) {
    return <section style={{backgroundColor: bg_color, height:"auto", padding: "10px"}}>
        ...
        <div className="container">
            {
                products.map(e => <Card sale_price={e.sale_price} regular_price={e.regular_price} key={e.name} img_src={e.thumb} name={e.name} off={Math.round(((e.regular_price - e.sale_price) / e.regular_price )* 100)} />)
            }
        </div>
    </section>
}